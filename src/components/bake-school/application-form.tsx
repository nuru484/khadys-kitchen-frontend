...
function FeeOption({
  ...
  return (
    <button
      ...
      className={cn(
        // Phones stack title/note above the amount. From sm, flex-wrap lets
        // the two share a row when the text fits on one line and drops the
        // amount below (text spreading the full row) only when it can't.
        "flex w-full cursor-pointer flex-col gap-1.5 rounded-[14px] border-[1.5px] px-4 py-3.5 text-left transition-colors sm:flex-row sm:flex-wrap sm:items-baseline sm:justify-between sm:gap-4",
        selected
